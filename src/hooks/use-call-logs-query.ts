  ...
  useEffect(() => {
    ...
    async function fetchCallLogs() {
      ...
      if (!assignmentRow?.assistant_id) {
        ...
        // No assignment means no call logs to show
        setData([]);
        setIsLoading(false);
        return;
      }
      ...
      console.log("[Assignment] assistant_id for user:", assignedAssistantId);

      // ── Step 2: Resolve price_per_minute from user's active subscription ──
      let pricePerMinute: number | null = null;
      ...
      // ── Step 3: Fetch CDRs filtered to the user's assigned assistant_id ───
      const { data: rows, error: fetchError } = await supabase
        .from("cdrs")
        .select("*")
